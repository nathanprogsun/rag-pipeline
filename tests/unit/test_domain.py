...
def test_dataset_creation():
    ds = Dataset(
        id=uuid.uuid4(), name="test", embed_model="text-embedding-3-small", embed_dim=1536
    )
    assert ds.vector_weight == 0.7  # default
    assert ds.prompt_template == ""  # empty default, app falls back to DEFAULT_PROMPT_TEMPLATE
