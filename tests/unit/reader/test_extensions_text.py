        ...
        async def test_text_adapter_chinese_encoding() -> None:
            ...
            async def test_text_adapter_gbk_encoding() -> None:
                ...
                async def test_text_adapter_md_base64_image_upload() -> None:
                    ...
                    async def uploader(
                        name: str, mime: str, buf: bytes
                    ) -> UploadedFileResult:
                        return {"key": "s3://bucket/key.png"}

                        result = await text_adapter(
                            md.encode("utf-8"), upload_file=uploader
                        )
                        ...
                        @pytest.mark.asyncio
                        async def test_text_adapter_md_base64_no_upload_strips() -> (
                            None
                        ):
                            ...
                            @pytest.mark.asyncio
                            async def test_text_adapter_invalid_bytes_falls_back_without_raising() -> (
                                None
                            ):
                                """含非法字节 → raw_text 内部兜底, 不抛异常。"""
                                buf = b"\xff\xfe\xfd broken"
                                # ``read_raw_text`` 内置 utf-8 + errors='replace' 兜底, text_adapter 不应抛。
                                result = await text_adapter(buf, encoding="utf-8")
