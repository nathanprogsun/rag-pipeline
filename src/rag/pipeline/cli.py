"""``rag-search`` Typer CLI: query → SearchResult via build_full_pipeline.

Mirrors ``rag-ingest`` CLI style (typer + typer.echo). Reads ``.env``
via pydantic-settings, constructs a ``PipelineDeps`` with default
...
def _build_embedder() -> Embeddings:
    ...
    Auto-disabled by pydantic-settings if OPENAI_EMBEDDING_API_KEY empty.
    """
...
def _emit_json(query: str, request: SearchRequest, response_text: str, citations: list, hits: list) -> None:
...
def main(
    ...
    # Build pipeline deps from real settings
    try:
    ...
    if output.lower() == "json":
        _emit_json(query, req, result.response, result.citations, result._intermediate_hits)
    else:
...
def _default_audit_path() -> Path | None:
    ...
    settings 中可能没有 cache_audit_path 字段 (取决于部署 profile),
    getattr 防御性 fallback.
    """
