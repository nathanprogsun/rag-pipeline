...
class SearchSubgraph:
    ...
    async def ainvoke(self, query: str) -> list[ScoredDocument]:
        ...
        Returns ``list[ScoredDocument]`` for this dataset. Empty list
        if both retrievers return empty / fail.
        """
        # 1. Validate request
        ...
        # 2. Vector + fulltext recall in parallel (asyncio.gather)
        ...
        # 3. Intra-fuse (Contract 1: query variant semantics — but here we
        # use it for "two sources within one dataset" which is FastGPT's
        # defaultRecall's `concatWeightedRecallLists` pattern).
        return intra_fusion(
            [vec_hits, ft_hits],
