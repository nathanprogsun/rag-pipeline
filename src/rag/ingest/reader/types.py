...
`UploadFileHandler` 是 `html2md` / 后续 reader 共享的异步上传回调签名,
接收 (filename, mime, bytes) 返回 `UploadedFileResult` TypedDict, 其中
``key`` 必填, ``previewUrl`` 可选。TypedDict 与 ``dict[str, str]`` 在
运行时结构兼容, mypy 静态类型与 Pydantic 校验均容许。
