...
不把 zip 解到磁盘, 直接 `ZipFile.read(name)` 在内存里读 bytes → XML,
省 IO 也避免副作用。只支持 ``extension == 'pptx'``; 其他 extension →
...
# 匹配 ppt/(notesSlides|slides)/(notesSlide|slide)\d+.xml
_ALL_FILES_RE = re.compile(r"^ppt/(notesSlides|slides)/(notesSlide|slide)\d+\.xml$")
# 匹配 ppt/slides/slide\d+.xml (必含子集)
...
def parse_office(
    ...
    *,
    ...
    # 落盘: mkstemp 拿 fd + path, 写 buffer, 立刻关闭 fd (后续用 path)。
    ...
    try:
        ...
        # 清理: 移除临时文件 (无论成功失败, finally 保证不泄漏)。
        _safe_unlink(tmp_path)
...
def _write_buffer_to_fd(fd: int, path: str, buffer: bytes) -> None:
    """写 buffer: 正常路径 `os.write`, 失败则用 `open` fallback。"""
    try:
        try:
            os.write(fd, buffer)
...
def _extract_pptx_text_inner(tmp_path: str, encoding: str) -> str:
    ...
    with zipfile.ZipFile(tmp_path, "r") as zf:
        ...
        # 校验: 必须至少有一个 slide\d+.xml (notesSlides 单独不算)。
...
def _read_member(zf: zipfile.ZipFile, name: str, encoding: str) -> bytes:
    """encoding 优先, decode 失败降级 utf-8。返回原始 bytes (留给 `ET.fromstring`)。"""
    raw = zf.read(name)
...
def _extract_slide_text(xml_bytes: bytes) -> str:
    """从单个 slide/notesSlide XML bytes 抽取 ``a:p`` 段文本 (段间 ``\n``)。

    行为:
    - 遍历所有 ``a:p`` (含嵌套, `ET.fromstring` 后直接迭代)
    - 过滤无 ``a:t`` 子节点的段落
    ...
    for para in root.iter(PARA_TAG):
        ...
        if not text_nodes:
            continue
        # 段内拼接: a:t 文本 join。Element 的 .text 即第一个文本子节点内容;
